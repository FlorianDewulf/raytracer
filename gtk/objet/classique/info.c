...
** Last update Fri Jun  1 22:31:25 2012 christophe del fabbro
*/
...
void		create_info(GtkWidget **g_sph_info)
{
  GtkWidget	*ptr_frame;
  GtkWidget	*ptr_spin;
  ...
  if (g_list->type == 3)
    ptr_frame = gtk_frame_new("Angle");
  else
    ptr_frame = gtk_frame_new("Rayon");
  ptr_spin = gtk_spin_button_new_with_range(1, 3000, 1);
  gtk_container_add(GTK_CONTAINER(ptr_frame), ptr_spin);
  gtk_box_pack_start(GTK_BOX((*g_sph_info)), ptr_frame, TRUE, TRUE, 0);
  g_signal_connect(G_OBJECT(ptr_spin), "value-changed",
...
void		create_button_info(GtkWidget **g_sph_info, GtkWidget *ptr_vbox)
{
...
void		info(GtkWidget *ptr_widget, gpointer data)
{
  GtkWidget	*tmp;
  static int	inf_cr;
  ...
  if (g_cr_inf == 0)
    {
      ...
      create_info(&g_sph_info);
      create_button_info(&g_sph_info, tmp);
      g_cr_inf = 1;
    }
